...
* **自動テスト**:
  `cargo test` を実行し、追加された画像へのリンクを含む合計 6 件の統合テスト（装飾、リンク、画像へのリンク、引用、リスト、テーブル）が 100% 合格することを確認しました。
* **手動検証**:
  日本語テキストや画像ファイルへのハイパーリンクを含む [test.md](file:///c:/DATA/Codes/mdocx/test.md) を使用し、`test.md` → `test.docx` → `test_back.md` の往復変換を実行。
  出力された `test_back.md` を元の `test.md` と比較し、日本語の文字化けがなく、ネストされたリスト、テーブル、複数行の Rust コードブロック、引用ブロック、画像へのリンクが完全に再現されていることを確認しました。
